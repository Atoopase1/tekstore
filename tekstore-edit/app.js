...
sb.auth.onAuthStateChange(async (event, session) => {
    ...
    if (event === 'SIGNED_IN' && session) {
        ...
        // an email verification or magic-link click. Regular page loads and normal
        // logins never have it, so this guard prevents a race with the startup
        // restoreSession() call that would wipe currentUser and hide the Account button.
        const isVerificationRedirect = window.location.hash.includes('access_token');
